...
import (
	...
	. "github.com/c00w/go-nfs-client-importable/protocol"
)

const (
	NfsReadBlockLen     = 1024 * 1024
	NFS4_MAX_OPERATIONS = 100 // Max number of operations in a compound - value copied from ganesha
)
...
func (c *NfsClient) runNfsTransaction(ops []Nfs_argop4, pathHint string) ([]Nfs_resop4, error) {
	c.mu.Lock()
	defer c.mu.Unlock() // The most convenient way to make it thread-safe
...
func (c *NfsClient) rebuildPath(opList []Nfs_argop4) []string {
	pathList := make([]string, 0, len(opList))
	path := []string{""} // For root
	for _, op := range opList {
		if op.Argop == OP_LOOKUP {
			path = append(path, string(op.U.(*LOOKUP4args).Objname))
		} else if op.Argop == OP_REMOVE { // End of path
			path = append(path, string(op.U.(*REMOVE4args).Target))
			pathList = append(pathList, strings.Join(path, "/"))
			path = path[:1]
		}
	}
	if len(path) != 1 {
...
func (c *NfsClient) DeleteFileBatch(pathList []string) error {
	lastOps := make([]Nfs_argop4, 0, len(pathList)*2)
	for _, path := range pathList {
		splits := splitPath(path)
		// Put the directory FH as the current one
		curOps := c.makePathLookupArgs(splits[0 : len(splits)-1])
		// Make the file claim (i.e. the file name on top of the directory FH)
		flClaim := Component4(splits[len(splits)-1])

		curOps = append(curOps,
			Nfs_argop4{
				Argop: OP_REMOVE,
				U: &REMOVE4args{
					Target: flClaim,
		...
		// Ganesha limits the number of ops in a single compound to 100, otherwise `NFS4ERR_RESOURCE` will be returned.
		if len(lastOps)+len(curOps) <= NFS4_MAX_OPERATIONS {
			lastOps = append(lastOps, curOps...)
		} else {
			if len(lastOps) > 0 {
				_, err := c.runNfsTransaction(lastOps, strings.Join(c.rebuildPath(lastOps), ","))
				if err != nil {
					return err
				}
			}
...
func (c *NfsClient) RemoveRecursive(ctx context.Context, path string) error {
	childCtx, cancelCtx := context.WithCancel(ctx)
	defer cancelCtx()
	pathCh := make(chan string, NFS4_MAX_OPERATIONS)
	...
	go func() {
		...
		for {
			...
			case <-childCtx.Done():
				return
			case path, more := <-pathCh:
				if !more {
					return
				}
				pathList := []string{path}
				for len(pathCh) > 0 {
					pathList = append(pathList, <-pathCh)
				}
				if len(pathList) > 0 {
					if err := c.DeleteFileBatch(pathList); err != nil {
						if !IsNfsError(err, ERROR_NOENT) { // Tolerate non-existing files so we can proceed
							delErr = err
							cancelCtx()
							return
						}
	...
	listErr := c.ListRecursive(childCtx, path, pathCh)
	close(pathCh)
	wg.Wait()
	if delErr != nil {
		return delErr
...
func (c *NfsClient) ListRecursive(ctx context.Context, path string, pathCh chan<- string) error {
	list, err := c.GetFileList(path)
	if err != nil {
		return err
	}
	...
	for _, fl := range list {
		...
		if fl.IsDir {
			err = c.ListRecursive(ctx, curPath, pathCh)
			if err != nil {
				return err
			}
		} else {
